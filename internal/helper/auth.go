...
func (a Auth) CreateHashedPassword(p string) (string, error) {
	...
	if len(p) < 6{
		return "", errors.New("password length should be at least of 6 characters long")
	}
	hashP, err := bcrypt.GenerateFromPassword([]byte(p),10)
	if err!= nil{
		log.Fatalf("password hash failed")
...
func (a Auth) GenerateToken(id uint, email string, role string) (string, error) {
	if id == 0 || email == "" || role == ""{
		return "", errors.New("Required fields are missing to genrate token")
	}
